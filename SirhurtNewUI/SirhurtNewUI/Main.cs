    ...
    public partial class Main : Form
    {
        ...
        public void TranslateLanguage(string Language)
        {
            if (Language == "" || string.IsNullOrEmpty(Language) || string.IsNullOrWhiteSpace(Language))
            {
                ...
                closeTabToolStripMenuItem.Text = "Close Editor";
            ...
            if (Language == "Russian" || Language == "russian")
            {
                ...
                closeTabToolStripMenuItem.Text = "Закрыть редактор"; //Close Editor
            ...
            if (Language == "French" || Language == "french")
            {
                ...
                closeTabToolStripMenuItem.Text = "Fermer l'éditeur"; //Close Editor
            ...
            if (Language == "Portuguese" || Language == "portuguese")
            {
                ...
                closeTabToolStripMenuItem.Text = "Fechar Editor"; //Close Editor
            ...
            if (Language == "German" || Language == "german")
            {
                ...
                closeTabToolStripMenuItem.Text = "Schließen Sie den Editor"; //Close Editor
            ...
            if (Language == "Polski" || Language == "polski")
            {
                ...
                saveToolStripMenuItem1.Text = "Zapisz"; //Save
                saveAsToolStripMenuItem.Text = "Zapisz jako"; //Save As
                closeTabToolStripMenuItem.Text = "Zamknij edytor"; //Close Editor
        ...
        public void InitSetup()
        {
            try
            {
                ...
                if (ColorClass.ToBool(MyIni.Read("loadIntellisense", "main")))
                    Settings.Default.yesnoornoplease = true;
        ...
        public Main()
        {
            ...
            TabContextMenu.Renderer = new BrowserMenuRenderer();
        ...
        {
            foreach (TabPage tab in customTabControl1.TabPages)
            {
                if (tab.Text.Contains("*"))
                {
                    DialogResult dialog = MessageBox.Show("Unsaved scripts detected would you like to save them?", "Sirhurt V4", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk);
                    if (dialog == DialogResult.Yes)
                    {
                        SaveTabsThatAreUnSaved(tab);
                    }
                }
            }
        ...
        private void Main_Load(object sender, EventArgs e)
        {
            ...
            Settings.Default.Oop = customTabControl1;
            ...
            customTabControl1.AddEvent("Script.lua");
            Debug("Setting global firstpage");
            Settings.Default.GodDamnBitch = customTabControl1.TabPages[0];
            ...
            if (saveTabsOnExitToolStripMenuItem.Checked)
            {
                try
                {
                    foreach (var Oop in Directory.GetFiles($"{Environment.CurrentDirectory}\\bin\\scripttemps"))
                    {
                        ...
                        customTabControl1.AddEvent(FileName, Base64decode(File.ReadAllText(Oop)));
                        File.Delete(Oop);
                    }
                }
                catch { }
        ...
        private void saveToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (!Directory.Exists($"{Environment.CurrentDirectory}\\Scripts"))
            {
                ...
                using (SaveFileDialog sfd = new SaveFileDialog())
                {
                    ...
                    {
                        File.WriteAllText(sfd.FileName, Settings.Default.KillMyself);
                    }
                }
            }
            else
            {
                using (StreamWriter Writer = new StreamWriter($"{Environment.CurrentDirectory}\\Scripts\\{Settings.Default.GodDamnBitch.Text}"))
                {
        ...
        private void oopToolStripMenuItem_MouseEnter(object sender, EventArgs e)
        {
            OldTabName = Settings.Default.FuckingBullShit;
        }
        ...
        private void ToolStripTextBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(Txt) || string.IsNullOrEmpty(Txt) || Txt == "")
                Settings.Default.GodDamnBitch.Text = OldTabName;

            Settings.Default.GodDamnBitch.Text = $"{Txt}.lua";
        }
        ...
        private async void runScriptToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (customTabControl1.GetWorkingTextEditor().Text == "Iris is the only god.")
            {
                IconPanel.Visible = false;
                PicEasterEgg.Visible = true;
                return;
            }
            SirHurtPipe(customTabControl1.GetWorkingTextEditor().Text);
        }
        ...
        private void ScriptList_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    customTabControl1.AddEvent(ScriptList.SelectedNode.Text, sr.ReadToEnd());
        ...
        private async void customTabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            await Task.Delay(50);
            Settings.Default.GodDamnBitch = customTabControl1.SelectedTab;
        }
        ...
        private void openFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog sfd = new OpenFileDialog())
            {
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    customTabControl1.AddEvent(Path.GetFileName(sfd.FileName), File.ReadAllText(sfd.FileName));
                }
            }
        }
        ...
        {
            customTabControl1.CloseTab(Settings.Default.GodDamnBitch);
        }
        ...
        {
            FindReplace MyFindReplace = new FindReplace();
            MyFindReplace = new FindReplace(customTabControl1.GetWorkingTextEditor());
        ...
        {
            customTabControl1.AddEvent("Settings.ini", File.ReadAllText($"{Environment.CurrentDirectory}\\bin\\schemes\\Settings.ini"));
        }
        ...
        {
            int i = 0;
            if (!Directory.Exists($"{Environment.CurrentDirectory}\\bin\\scripttemps"))
                Directory.CreateDirectory($"{Environment.CurrentDirectory}\\bin\\scripttemps");
            foreach (var a1 in customTabControl1.TabPages)
            {
                TabPage CurrTab = a1 as TabPage;
                string TabName = CurrTab.Text.Replace("*", "");
                string TabEditorText = "";
                try
                {
                    TabEditorText = CurrTab.Controls[0].Text;
                ...
                if (TabName != "" || !string.IsNullOrEmpty(TabName))
                {
                    bool Exists = File.Exists($"{Environment.CurrentDirectory}\\bin\\scripttemps\\{TabName}");
                    if (Exists)
                    {
                        TabName = TabName.Replace(".lua", $"_{i.ToString()}.lua");
                    }
                    
                    using (StreamWriter writer = new StreamWriter($"{Environment.CurrentDirectory}\\bin\\scripttemps\\{TabName}"))
                    {
                        writer.Write(Base64encode(TabEditorText));
                    }
                }
                i++;
            }
        ...
        {
            Settings.Default.FuckAllOfYou.Text = "";
        }
